local a a={cache={}, load=function(b)if not a.cache[b]then a.cache[b]={c=a[b]()}end return a.cache[b].c end}do function a.a()
    ...
    local G="SwebUi/"..z.."/Assets/."..A.."-"..r..".png"
    ...
    if not H then
    warn("[ SwebUi.Creator ]  '"..identifyexecutor().."' doesnt support the URL Images. Error: "..J)
    ...
    return[[
    {
        ...
        "version": "1.6.52",
        "main": "./dist/main.lua",
        "repository": "https://github.com/Footagesus/SwebUi",
    ...
    local aj=a.load'l'.Init(nil,ag.SwebUi.ScreenGui.KeySystem)
    ...
    local h=ag.SwebUi.Services[g.Type]
    if h then
    local i={}
    ...
    local ag=a.load'l'.Init(nil,ae.SwebUi.ScreenGui.Popups)
    ...
    warn"[ SwebUi.ConfigManager ] Window.Folder is not specified."
    return false
    end
    
    ab=ae
    ac.Folder=ab.Folder
    ac.Path="SwebUi/"..tostring(ac.Folder).."/config/"
    
    if not isfolder("SwebUi/"..ac.Folder)then
    makefolder("SwebUi/"..ac.Folder)
    if not isfolder("SwebUi/"..ac.Folder.."/config/")then
    makefolder("SwebUi/"..ac.Folder.."/config/")
    end
    end
    ...
    an.Load(ai,am.Content,an.Elements,ah.Window,ah.SwebUi,function()
    ...
    al.ElementsModule.Load(al,al.UIElements.ContainerFrame,al.ElementsModule.Elements,Window,SwebUi,nil,al.ElementsModule,ak)
    ...
    if not aC then
    warn("[ SwebUi.Background ]  Failed to download video: "..tostring(aD))
    return
    end
    end
    ...
    ImageTransparency=an.Transparent and am.SwebUi.TransparencyValue or 0
    },{
    e
    })
    end
    ...
    an.Destroyed=true
    task.wait(0.4)
    am.SwebUi.ScreenGui:Destroy()
    am.SwebUi.NotificationGui:Destroy()
    am.SwebUi.DropdownGui:Destroy()
    ...
    an.Transparent=j
    am.SwebUi.Transparent=j
    
    an.UIElements.Main.Background.ImageTransparency=j and am.SwebUi.TransparencyValue or 0
    ...
    return am.SwebUi.UIScale
    end
    
    function an.SetUIScale(i,j)
    am.SwebUi.UIScale=j
    ag(am.SwebUi.ScreenGui.UIScale,.2,{Scale=j},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
    return an
    end
    ...
    local i=a.load'R'
    local j=a.load'S'
    local l=i.Init(an,am.SwebUi,am.Parent.Parent.ToolTips)
    ...
    return l.New(p,am.SwebUi.UIScale)
    end
    ...
    return j.New(p,an.UIElements.SideBar.Frame,an.Folder,am.SwebUi.UIScale,an)
    end
    ...
    Name="SwebUi/Notifications",
    Parent=ao,
    IgnoreGuiInset=true,
    })
    aa.DropdownGui=aj("ScreenGui",{
    Name="SwebUi/Dropdowns",
    Parent=ao,
    IgnoreGuiInset=true,
    })
    an(aa.ScreenGui)
